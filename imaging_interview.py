# -*- coding: utf-8 -*-
"""
Created on Wed Oct 27 01:18:31 2021
...
def preprocess_image_change_detection(img, gaussian_blur_radius_list=None, black_mask=(5, 10, 5, 0)):
    ...
    if gaussian_blur_radius_list is not None:
        for radius in gaussian_blur_radius_list:
            gray = cv2.GaussianBlur(gray, (int(radius), int(radius)), 0)
...
def compare_frames_change_detection(prev_frame, next_frame, min_contour_area):
    ...
    (_, thresh_img) = cv2.threshold(frame_delta, 45, 255, cv2.THRESH_BINARY) #(thresh_val, thresh_image)
    thresh = cv2.dilate(thresh_img, None, iterations=1)  
    ...
    for c in cnts:
        ...
        res_cnts.append(c)
        score += cv2.contourArea(c) #sum of areas of detected contours
